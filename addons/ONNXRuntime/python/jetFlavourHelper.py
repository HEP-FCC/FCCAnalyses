...
class JetFlavourHelper:
    def __init__(self, coll, jet, jetc, tag=""):
        ...
        self.definition[f"dqdxHandler{self.tag}"] = \
            f"TrackUtils::get_dqdxHandler({self.dndx}, _{self.dndx}_track.index)"

        self.definition[f"pfcand_dndx{self.tag}"] = \
            f"JetConstituentsUtils::get_dndx({self.const}, dqdxHandler{self.tag}, {self.pftrack}, pfcand_isChargedHad{self.tag})"
