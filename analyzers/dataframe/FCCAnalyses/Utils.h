...
namespace FCCAnalyses {
  ...
  /// - select indices of selected objects from list of inputs and indices to
  /// study,
  ///   return index ist
  /// - select indicises of objects passing for a set of input lists (nested
  /// selection) This allows to consistently apply the same selection in
  /// multiple ways, and to easily write new cuts without having to write a lot
  /// of boilerplate code.
  ...
  ///
  /// One pattern is to create a selector class inheriting from
  /// `selByPredicate`, where the selection function is specified within the
  /// constructor. Constructor arguments can be passed via lambda capture or
  /// `std::bind` to parametrise cuts where needed. This can look like
  /// @code
  ...
  ///     myPtCut(double par1, int par2):
  ///     selByPredicate<edm4hep::MCParticleData>(
  ///        std::bind(doesThisPass,std::placeholders::_1, par1, par2)){
  ///     }
  /// };
  /// @endcode
  /// Examples for practical usage can be found in `MCParticle.h`, see for
  /// example #sel_pt. In the python steering, you would then typically
  /// instantiate the derived cut classes, for example
  /// @code
  ...
  /// @code
  /// df =
  /// df.Define("particle_indices_passing","myPtCut(10)(particle_indices_to_check,
  /// allParticles)")
  /// @endcode
  template <class thingToSelect> struct selByPredicate {
    ...
    selByPredicate(std::function<bool(const thingToSelect &)> thePredicate)
    ...
    /// @brief Select (several) lists of passing objects from (several) lists of
    /// candidates, return by lists of indices. Used to select on several
    /// (similar) lists at once. For example: Track selection within (many)
    /// vertex candidates.
    ...
    operator()(const ROOT::VecOps::RVec<ROOT::VecOps::RVec<int>> &setsOfIndices,
               const ROOT::VecOps::RVec<thingToSelect> &in) {
