//library with extra functions needed by custom FCC-hh analysis, such as HHbbZZ(llvv) analysis
...
namespace AnalysisFCChh{
  ...
  // ROOT::VecOps::RVec<float> get_pt_test(ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> in);
	...
	//struct to use for a pair of two reco particles, to make sure the correct ones stay together
	...
	ROOT::VecOps::RVec<RecoParticlePair> get_first_pair(ROOT::VecOps::RVec<RecoParticlePair> pairs); //can use to get leading pair if the inputs to pair finding fct were pT sorted
	
	//functions to separate the pair again - ONLY DOES THIS FOR THE FIRST PAIR IN THE VECTOR
	...
	// ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> getOSPair(ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> leptons_in);
	ROOT::VecOps::RVec<RecoParticlePair> getBestOSPair(ROOT::VecOps::RVec<RecoParticlePair> electron_pairs, ROOT::VecOps::RVec<RecoParticlePair> muon_pairs); //closest to Z mass
	ROOT::VecOps::RVec<RecoParticlePair> getLeadingPair(ROOT::VecOps::RVec<RecoParticlePair> electron_pairs, ROOT::VecOps::RVec<RecoParticlePair> muon_pairs); //pair with leading pT(pair)
  ...
  ROOT::VecOps::RVec<edm4hep::MCParticleData> getChadron(ROOT::VecOps::RVec<edm4hep::MCParticleData> truth_particles, ROOT::VecOps::RVec<podio::ObjectID> parent_ids);
	...
	//merge two four vectors into one to create a new particle (follow vector structure to be able to use with other RecoParticle fcts easily like get_pt etc.)
	...
	// stransverse mass mT2 : https://www.hep.phy.cam.ac.uk/~lester/mt2/#Alternatives
	// ROOT::VecOps::RVec<float> get_mT2(ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> particle_1, ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> particle_2, ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> MET_obj);
	// ROOT::VecOps::RVec<float> get_mT2_125(ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> particle_1, ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> particle_2, ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> MET_obj);

	//angular distances:function can return dR, dEta, dPhi for any two fully reconstructed particles that have a full 4 vector
