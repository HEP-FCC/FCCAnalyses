...
namespace myUtils{
  ...
  /// ComputeMomentaAtVertex controls whether updated_track_momentum_at_vertex is filled for each
  /// returned vertex: it requires building a Delphes VertexMore object, whose cost scales as
  /// O(ntracks^3) and dominates the total runtime for events with many tracks. Set to false when
  /// only the vertex position/chi2/primary flag is needed (e.g. hasPV()) -- see
