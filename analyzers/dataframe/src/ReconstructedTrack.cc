...
ROOT::VecOps::RVec<edm4hep::TrackState>  Intersection ( const ROOT::VecOps::RVec<edm4hep::TrackState>&  Col1 ,
  ...
  for (auto & track1: Col1) {
     ...
     if ( isInBoth) result.push_back( track1 );
  }
...
ROOT::VecOps::RVec<edm4hep::TrackState>  Remove ( const ROOT::VecOps::RVec<edm4hep::TrackState>&  Subset ,
...
ROOT::VecOps::RVec<edm4hep::TrackState>  Merge ( const ROOT::VecOps::RVec<edm4hep::TrackState>&  Col1 ,
                                                 const ROOT::VecOps::RVec<edm4hep::TrackState>&  Col2 ) {
  ...
  for (auto & track2: Col2) {
     ...
     if ( ! isInBoth) result.push_back( track2 );
  }
...
ROOT::VecOps::RVec<edm4hep::TrackState> TrackStates_at_IP( const ROOT::VecOps::RVec< edm4hep::TrackData >&  in,
                                                           const ROOT::VecOps::RVec<edm4hep::TrackState>& trackstates)  {

 ROOT::VecOps::RVec<edm4hep::TrackState> result;
 int n_trackstates = trackstates.size();
 int nm4 = n_trackstates % 4;
 if ( nm4 != 0 ) std::cout << " ================= modulo 4 != 0 " << nm4 << std::endl;
...
ROOT::VecOps::RVec<int>  get_indices ( const ROOT::VecOps::RVec<edm4hep::TrackState>&  some_tracks ,
...
ROOT::VecOps::RVec<float> tracks_length( const ROOT::VecOps::RVec<int>& track_indices,
...
ROOT::VecOps::RVec<float> tracks_length( const ROOT::VecOps::RVec<edm4hep::TrackState>&  some_tracks ,
					 const ROOT::VecOps::RVec<edm4hep::TrackState>&  FullTracks ,
					 const ROOT::VecOps::RVec<float> &length ) {    // collection EFlowTrack_L 
...
ROOT::VecOps::RVec<float> tracks_TOF( const ROOT::VecOps::RVec<int>& track_indices,
					...
					const ROOT::VecOps::RVec<edm4hep::TrackerHitData> &trackerhits ) 
{
 ...
 for (int i=0; i < track_indices.size(); i++) {
   int tk_idx = track_indices[i];		// index of the track in EFlowTrack_1
            ...
            int tk_jdx = -1;
            // find the index of the track in Eflowtrack (in principle, it is the same as tk_idx)
...
ROOT::VecOps::RVec<float> tracks_TOF( const ROOT::VecOps::RVec<edm4hep::TrackState>&  some_tracks ,
					const ROOT::VecOps::RVec<edm4hep::TrackState>&  FullTracks ,
					const ROOT::VecOps::RVec<edm4hep::TrackData> &trackdata, // Eflowtrack
                                        const ROOT::VecOps::RVec<edm4hep::TrackerHitData> &trackerhits )
{
...
ROOT::VecOps::RVec<float> tracks_dNdx( const ROOT::VecOps::RVec<int>& track_indices,
                                                          const ROOT::VecOps::RVec<edm4hep::TrackData> &trackdata, // Eflowtrack
...
ROOT::VecOps::RVec<float> tracks_dNdx( const ROOT::VecOps::RVec<edm4hep::TrackState>&  some_tracks ,
                                        const ROOT::VecOps::RVec<edm4hep::TrackState>&  FullTracks ,
                                                          const ROOT::VecOps::RVec<edm4hep::TrackData> &trackdata, // Eflowtrack
                                                          const ROOT::VecOps::RVec<edm4hep::Quantity> &dNdx)        // ETrackFlow_2
{
