...
int AnalysisFCChh::checkWDecay(edm4hep::MCParticleData truth_W, ROOT::VecOps::RVec<podio::ObjectID> daughter_ids, ROOT::VecOps::RVec<edm4hep::MCParticleData> truth_particles){
	...
	if(children_size < 1){
		std::cout << "Error in checkWDecay! Checking W with no daughters, returning 0."<< std::endl;
		return 0;
	}

	//have at least 1 child -> if its also a W, continue the chain -> skip the intermediate Ws, and those that radiated photon
	...
	if(children_size!= 2){
		std::cout << "Error in checkWDecay! Found unexpected W decay, please check."<< std::endl;
	...
	//debug
	// std::cout << "Checking W decay: PDGID 1 = " << child_1.PDG << " and PDGID 2 = " <<  child_2.PDG << std::endl;
	...
	for (auto & truth_part: truth_particles) {
		if (isTop(truth_part)){
			...
			if (isW(child_1) && !isW(child_2)){
				...
				std::cout << "Warning! Found two or Ws from top decay. Please check. Skipping top." << std::endl;
				continue;
			}
...
//check Higgs decay: use to see if can improve stats for single Higgs bkg with exclusive samples
int AnalysisFCChh::findHiggsDecayChannel(ROOT::VecOps::RVec<edm4hep::MCParticleData> truth_particles, ROOT::VecOps::RVec<podio::ObjectID> daughter_ids){
	...
	for (auto & truth_part: truth_particles) {
		if (isH(truth_part)){
			...
			//Higgs decay types:
			// 1: Hbb, 2: HWW, 3: Hgg, 4: Htautau, 5: Hcc, 6:HZZ, 7:Hyy, 8:HZy, 9:Hmumu, 10:Hss
			...
			if (isb(child_1) && isb(child_2)){
			...
			else if ( (isZ(child_1) && isPhoton(child_2)) || (isZ(child_2) && isPhoton(child_1)) ){
				higgs_decay_type = 8;
			}
				...
				std::cout << "Pdg ids are " << child_1.PDG << " , " << child_2.PDG << std::endl;
				continue;
			}		
		}
	}
...
ROOT::VecOps::RVec<edm4hep::MCParticleData> AnalysisFCChh::get_truth_Z_decay(ROOT::VecOps::RVec<edm4hep::MCParticleData> truth_particles, ROOT::VecOps::RVec<podio::ObjectID> daughter_ids, TString decay){
	...
	for (auto & truth_part: truth_particles) {
		if (isZ(truth_part)){
			...
			// std::cout << "Found Z with children with indices " << first_child_index << " , " << last_child_index << std::endl;
			// std::cout << "number of Z daughters:" << last_child_index - first_child_index << std::endl; 
...
//for testing: same as above, but not removing already matched reco objects, allowing double match
ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> AnalysisFCChh::find_reco_matches_no_remove(ROOT::VecOps::RVec<edm4hep::MCParticleData> truth_parts, ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> reco_particles, float dR_thres){
