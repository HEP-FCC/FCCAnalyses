...
// to MC particles
//
ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> Vertexing::SelPrimaryTracks ( ROOT::VecOps::RVec<int> recind,
...
edm4hep::VertexData  Vertexing::VertexFitter( int Primary,
  ...
  int Ntr = tracks.size();
  if ( Ntr <= 1) return result;   // can not reconstruct a vertex with only one track...
