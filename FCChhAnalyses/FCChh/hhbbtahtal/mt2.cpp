...
class asymm_mt2_lester_bisect {
 public:
  ...
  static double get_mT2_Sq( // returns square of asymmetric mT2 (which is >=0), or returns a negative number (such as MT2_ERROR) in the case of an error.
    ...
    /*
    std::cout<<mVis1 <<std::endl;
    std::cout<<pxVis1 <<std::endl;
    std::cout<<pyVis1 <<std::endl;
    std::cout<<mVis2 <<std::endl;
    std::cout<<pxVis2 <<std::endl;
    std::cout<<pyVis2 <<std::endl;
    std::cout<<pxMiss <<std::endl;
    std::cout<<pyMiss <<std::endl;
    std::cout<<mInvis1 <<std::endl;
    std::cout<<mInvis2 <<std::endl;
    std::cout<<desiredPrecisionOnMT2 <<std::endl;
    std::cout<<useDeciSectionsInitially <<std::endl;
    */
...
// #define LESTER_DBG 1
 ...
 private:
  ...
  static const Lester::EllipseParams helper(const double mSq, // The test parent-mass value (squared)
       ...
       const double mqSq, // The mass of the invisible particleasymm_mt2_lester_bisect
       const double pxmiss, const double pymiss
             ) {
...
extern "C" {
    asymm_mt2_lester_bisect* asymm_mt2_lester_bisect_new(){ return new asymm_mt2_lester_bisect(); }
    
    double asymm_mt2_lester_bisect_get_mT2(
          asymm_mt2_lester_bisect* asym_mt2,
          const double mVis1, const double pxVis1, const double pyVis1,
          const double mVis2, const double pxVis2, const double pyVis2,
          const double pxMiss, const double pyMiss,
          const double mInvis1, const double mInvis2,
    ...
    { 
        return asym_mt2->get_mT2(
            mVis1, pxVis1, pyVis1,
            mVis2, pxVis2, pyVis2,
            pxMiss, pyMiss,
            mInvis1, mInvis2,
            desiredPrecisionOnMT2, // This must be non-negative.  If set to zero (default) MT2 will be calculated to the highest precision available on the machine (or as close to that as the algorithm permits).  If set to a positive value, MT2 (note that is MT2, not its square) will be calculated to within +- desiredPrecisionOnMT2. Note that by requesting precision of +- 0.01 GeV on an MT2 value of 100 GeV can result in speedups of a factor of ...
            useDeciSectionsInitially // If true, interval is cut at the 10% point until first acceptance, which gives factor 3 increase in speed calculating kinematic min, but 3% slowdown for events in the bulk.  Is on (true) by default, but can be turned off by setting to false.
        ); 
    }
}
