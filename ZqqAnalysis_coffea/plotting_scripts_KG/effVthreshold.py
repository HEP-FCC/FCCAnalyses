...
print('length of thresholds_s = '+str(np.size(thresholds_s)))
print('length of fpr_s = '+str(np.size(fpr_s)))


plt.plot(tpr_u, thresholds_u, label='up quarks', linestyle='-', color='b')
plt.plot(tpr_d, thresholds_d, label='down quarks', linestyle='-', color='r')
plt.plot(tpr_s, thresholds_s, label='strange quarks', linestyle='-', color='c')
...
#plt.yscale('log')
plt.legend(loc='best')
plt.savefig('../ConvNet/plots/LodeNet_effvsthreshold.png')
