...
def Writeh5(output, name,folder):
    ...
    with h5.File(os.path.join(folder,'{0}.h5'.format(name)), "w") as fh5:
        ...
        #dset = fh5.create_dataset("nConsti", data=output['nConsti'].value)
...
class MyProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        print('/////////////////')
        print(jets.x)
        up_mask = (np.abs(jets.partonFlavour[:,0])==2)&(np.abs(jets.partonFlavour[:,0])==2)
        down_mask = (np.abs(jets.partonFlavour[:,0])==1)&(np.abs(jets.partonFlavour[:,0])==1)
        strange_mask = (np.abs(jets.partonFlavour[:,0])==3)&(np.abs(jets.partonFlavour[:,0])==3)
        charm_mask = (np.abs(jets.partonFlavour[:,0])==4)&(np.abs(jets.partonFlavour[:,0])==4)
        bottom_mask = (np.abs(jets.partonFlavour[:,0])==5)&(np.abs(jets.partonFlavour[:,0])==5)
        ...
        '''
        for i in range(len(firstjetGenPartsf.phi)):
            nphisto[i,0] = np.histogram2d(neutralKaon_phi_diff[i], neutralKaon_theta_diff[i], bins=bins)[0]
            nphisto[i,1] = np.histogram2d(chargedKaon_phi_diff[i], chargedKaon_theta_diff[i], bins=bins)[0]
            nphisto[i,2] = np.histogram2d(neutralPion_phi_diff[i], neutralPion_theta_diff[i], bins=bins)[0]
            nphisto[i,3] = np.histogram2d(chargedPion_phi_diff[i], chargedPion_theta_diff[i], bins=bins)[0]
            nphisto[i,4] = np.histogram2d(electron_phi_diff[i], electron_theta_diff[i], bins=bins)[0]
            nphisto[i,5] = np.histogram2d(muon_phi_diff[i], muon_theta_diff[i], bins=bins)[0]
            nphisto[i,6] = np.histogram2d(photon_phi_diff[i], photon_theta_diff[i], bins=bins)[0]
            nphisto[i,7] = np.histogram2d(protonNeutron_phi_diff[i], protonNeutron_theta_diff[i], bins=bins)[0]
...
'''
#events = processor.LazyDataFrame(file['events'], entrystop=10000)
...
print(output)
print(len(output['histos'].value))
Writeh5(output, 'p8_ee_Zbb_ecm91_images_p5', 'h5_output')

###Writeh5(out['ML'], 'test', 'h5_output')
