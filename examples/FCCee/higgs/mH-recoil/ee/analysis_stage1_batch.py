'''
Example Higgs recoil mass analysis with electrons. Shows how to run on HTCondor.
'''

# Mandatory: List of processes
processList = {
    # Run the full statistics in 20 jobs in output dir
    # <outputDir>/p8_ee_ZZ_ecm240/chunk<N>.root
    "p8_ee_ZZ_ecm240": {
        "chunks": 20
    },
    # Run the full statistics in 20 jobs in output dir
    ...
    # Run the full statistics in 20 jobs in output dir
    # <outputDir>/p8_ee_ZH_ecm240/chunk<N>.root
    "p8_ee_ZH_ecm240": {
        "chunks": 20
    },
}
...
# Optional: Provide additional analyzers for your analysis
# includePaths = ['additional_analyzers.h']
...
# Optional computing account when running on HTCondor, default is
# group_u_FCC.local_gen
compGroup = "group_u_FCC.local_gen"

# Optional output directory on eos, if specified files will be copied there
# once the batch job is done, default is empty
outputDirEos = "/eos/experiment/fcc/ee/analyses/case-studies/higgs/mH-recoil/"

# Optional type for eos, needed when <outputDirEos> is specified. The default
# is FCC eos which is eospublic
eosType = "eospublic"
...
class RDFanalysis:
    ...
    Mandatory: RDFanalysis class where the user defines the operations on the
               dataframe and outputs of the analysis.
    '''
    def analysers(df):
        '''
        Mandatory: Method where the operations on the individual dataframe
                   columns are defined, please make sure you return the last
                   dataframe, in this example it is "df2"
        '''
        df2 = (
            df
            ...
            .Define(
                "selected_electrons", "ReconstructedParticle::sel_pt(10.)(electrons)"
            )
            # create branch with electron transverse momentum
            .Define(
                "selected_electrons_pt",
                "ReconstructedParticle::get_pt(selected_electrons)",
            )
            # create branch with electron rapidity
            .Define(
                "selected_electrons_y",
                "ReconstructedParticle::get_y(selected_electrons)",
            )
            # create branch with electron total momentum
            .Define(
                "selected_electrons_p",
                "ReconstructedParticle::get_p(selected_electrons)",
            )
            # create branch with electron energy
            .Define(
                "selected_electrons_e",
                "ReconstructedParticle::get_e(selected_electrons)",
            )
            # find zed candidates from  di-electron resonances
            .Define(
                "zed_leptonic",
                "ReconstructedParticle::resonanceBuilder(91)(selected_electrons)",
            )
            ...
            .Define(
                "zed_leptonic_charge", "ReconstructedParticle::get_charge(zed_leptonic)"
            )
    ...
    def output():
        '''
        Mandatory: method to specify columns to be saved, please make sure you
                   return the branch list as a python list
        '''
        branchList = [
            "selected_electrons_pt",
            "selected_electrons_y",
