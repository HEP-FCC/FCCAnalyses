...
// Sigma of D0 :
events->Draw("TMath::Sqrt(RP_TRK_D0_var)","TMath::Sqrt(RP_TRK_D0_var) < 1e-2") ;	// the mean should be a few microns
...
events->Draw("RP_TRK_D0 / TMath::Sqrt(RP_TRK_D0_var)") ;  // sigma = 1 as it should
...
events->Draw("TMath::Sqrt(RP_TRK_Z0_var)") ;  // mean = 4e-3 : the variance is in mm-squared
events->Draw("RP_TRK_Z0 / TMath::Sqrt(RP_TRK_Z0_var)") ;  // std dev = 1
...
events->Draw(" ( TMath::Abs(RP_TRK_omega) - 1e-3*0.3*2*(1./RP_MC_tlv.Pt()) ) / TMath::Sqrt( RP_TRK_omega_var)");   
	// sigma = 1, OK
